...
static int write_body(Agraph_t * g, iochan_t * ofile)
{
	...
	if (write_node_test(g, n))
	    CHKRV(write_node(g, n, ofile, dd ? dd->dict.n : 0));
	prev = n;
	for (e = agfstout(g, n); e; e = agnxtout(g, e)) {
	    if (prev != aghead(e) && write_node_test(g, aghead(e))) {
		CHKRV(write_node(g, aghead(e), ofile, dd ? dd->dict.n : 0));
		prev = aghead(e);
	    }
	    if (write_edge_test(g, e))
		CHKRV(write_edge(g, e, ofile, dd ? dd->dict.e : 0));
	}
...
static void before_write(Agraph_t *g)
{
    ...
    wptr = g->clos->wr_info = agalloc(g,sizeof(struct graphviz_write_info));
    wptr->preorder_number = agnnew(g, g->clos->seq[AGRAPH]+1, uint64_t);
    wptr->node_last_written = agnnew(g, g->clos->seq[AGNODE]+1, uint64_t);
    wptr->edge_last_written = agnnew(g, g->clos->seq[AGEDGE]+1, uint64_t);
    subgdfs(g,1);
}
...
static void after_write(Agraph_t *g)
{
    ...
    g->clos->wr_info = NULL;
    agfree(g,wptr->preorder_number);
    agfree(g,wptr->node_last_written);
    agfree(g,wptr->edge_last_written);
    agfree(g,wptr);
}
