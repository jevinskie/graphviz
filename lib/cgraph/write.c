...
static int write_body(Agraph_t *g, iochan_t *ofile,
                      struct graphviz_write_info *wr_info);

static struct graphviz_write_info before_write(Agraph_t *);
static void after_write(Agraph_t *, struct graphviz_write_info);
...
static struct graphviz_write_info before_write(Agraph_t *g) {
    struct graphviz_write_info wr_info = {0};
    set_attrwf(g, true, false);
    
    wr_info.preorder_number = agnnew(g, g->clos->seq[AGRAPH] + 1, uint64_t);
    wr_info.node_last_written = agnnew(g, g->clos->seq[AGNODE] + 1, uint64_t);
    wr_info.edge_last_written = agnnew(g, g->clos->seq[AGEDGE] + 1, uint64_t);
    subgdfs(g, 1, &wr_info);
    return wr_info;
}

static void after_write(Agraph_t *g, struct graphviz_write_info wr_info) {
    agfree(g, wr_info.preorder_number);
    agfree(g, wr_info.node_last_written);
    agfree(g, wr_info.edge_last_written);
}
